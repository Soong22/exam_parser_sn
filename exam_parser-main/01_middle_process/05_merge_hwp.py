...
# ======================================================
XHTML_DIR_IN = Path(r"00_PyHWP/data/01_html")                        # 각 문제 폴더에 index.xhtml 존재
JSON_DIR_IN  = Path(r"01_middle_process/data/choice_process")  # 병합 대상 JSON 폴더(재귀)
OUT_DIR      = Path(r"text_merge_test/data/merge_html")                   # 출력 루트
...
def extract_plain_from_xhtml(xhtml_html: str, p_only: bool = True) -> str:
    ...
    if p_only:
        ...
        for br in soup.find_all("br"):
            br.replace_with("\n")
        txt = "".join(soup.strings).replace("\t", "").replace("\r", "")
    return txt
...
# ===== 숫자/OCR 혼입 케이스 판단 =====
_re_hangul_digit_mix = re.compile(r"[가-힣]\d+|[가-힣]\d+[가-힣]|\d+[가-힣]")
...
def _xhtml_key_from_path(p: Path) -> str:
    name = unicodedata.normalize("NFC", p.parent.name)
...
def folder_main():
    ...
    # 모든 index.xhtml 수집
    xhtml_files = [p for p in XHTML_DIR_IN.rglob("index.xhtml")]
    if not xhtml_files:
        print(f"[WARN] index.xhtml 없음: {XHTML_DIR_IN}")
