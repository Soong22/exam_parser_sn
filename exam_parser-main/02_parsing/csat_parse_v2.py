...
def find_answers_by_stem(answer_path: str) -> Dict[str, Dict[str, Any]]:
    ...
    # 1) 파일인 경우: 그 파일 하나만 읽어서 data_id 기반 STEM 매핑
    if os.path.isfile(ap_norm):
        ans_map, exp_map, meta_top, perq_map = load_answer_bundle(ap_norm)

        # 단일 파일에 여러 과목/라인이 섞여 있을 수 있으므로,
        # load_answer_bundle이 반환한 맵에서 key(문제번호)만 있고 STEM 정보가 없다는 문제가 생김.
        # -> 파일 본문을 직접 다시 훑어서 STEM(=data_id 앞 4자리)별로 묶어주자.
    ...
    # 2) 디렉터리인 경우: 기존 로직 (파일명 앞 4자리 STEM)
    ...
    for fn in names:
        ...
        if "정답" not in norm: 
            continue
        m = re.match(r"^(\d{4})", norm)  # 파일명 앞 4자리 STEM
        if not m: 
            continue
        stem = m.group(1)
...
# ===== ★ 라운드/인덱스 계산 유틸 =====
def _compute_round_indices(questions: List[Question]) -> None:
    """
    문제번호가 다시 1이 나오면 새 라운드 시작.
    각 문항에 _round, _idx_in_round, _idx_global을 채운다.
    """
    round_no = 1
    idx_in_round = 0
    idx_global = 0
...
def build_kt_jsonl(
    ...
    # ---- 라운드/인덱스 채우기
    _compute_round_indices(questions)

    # --- 파일명으로 파싱한 기본 메타(베이스 id 등) ---
    meta = _parse_meta_from_filename(meta_src_path)
    base_id = meta.get("data_id", "")  # 예: '1234'
    ...
    with open(out_jsonl_path, "w", encoding="utf-8") as fw:
        for q in questions:
            ...
            # ==== ★ ID 체계 ====
            # 기존 base_id도 전역 인덱스를 붙여서 ****_0001, ****_0002, ...
            ...
            # --- 플레이스홀더 변환 (태그/타이틀도 새 ID 반영) ---
            for kind, idx in ph_order:
                # 태그 접두는 '기존 data_id도 ****_000X' 요구대로 base_id_with_global 사용
