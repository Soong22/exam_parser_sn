...
def load_answer_bundle(answer_path: str) -> Tuple[Dict[str, Any], Dict[str, str]]:
    """정답/해설 JSONL 묶음 로드 (BOM 안전)"""
    ...
    def merge_ans(k, v):
        ...
        if isinstance(v, dict):
            if "정답" in v: ans_map[kk] = v["정답"]
            if "해설" in v and v["해설"] not in (None, ""): exp_map[kk] = str(v["해설"])
        else:
            ans_map[kk] = v
...
def find_answers_by_stem(answer_dir: str) -> Dict[str, Dict[str, Any]]:
    ...
    - 파일명 유니코드 정규화(NFKC)로 매칭해 mac/윈도우 차이 완화
    - 우선순위: 'STEM-...-정답.jsonl' > 'STEM...정답...jsonl'
    """
    mapping: Dict[str, Dict[str, Any]] = {}
    ...
    for fn in names:
        ...
        if not norm.lower().endswith(".jsonl"):
            continue
        if "정답" not in norm:
            continue
        ...
        ap = os.path.join(answer_dir, fn)  # 실제 파일 접근은 원본 이름 사용
        ans_map, exp_map = load_answer_bundle(ap)

        prev = mapping.get(stem)
        this_is_strict  = norm.endswith("-정답.jsonl")
        prev_is_strict  = bool(prev and unicodedata.normalize("NFKC", os.path.basename(prev["path"])).endswith("-정답.jsonl"))

        if (not prev) or (this_is_strict and not prev_is_strict):
            mapping[stem] = {"ans": ans_map, "exp": e_map if (e_map := exp_map) else {}, "path": ap}
...
def build_kt_jsonl(
    ...
    year = meta.get("year", "")
    exam_name = meta.get("exam", "")
    subject = meta.get("subject", "")
    exam_type = meta.get("type", "")
    ...
    cat_main, cat_sub = _guess_category(subject)
    ...
    with open(out_jsonl_path, "w", encoding="utf-8") as fw:
        for q in questions:
            ...
            # 선택지 병합 텍스트에서는 정렬은 숫자값 기준, 라벨은 원형 유지
            combined_text = "\n".join(
                [body_pre, view_pre]
            ...
            # ✅ 플레이스홀더 등장 순서대로 태그 생성/저장하고 매번 asset_seq 증가
            for kind, idx in ph_order:
                ...
                if kind == "IMG":
                    ...
                    # ✅ 다양한 키 커버: file_name > image_path > img_path > src
                    ...
                    # ✅ 다양한 키 커버: file_name > image_path > table_img_path > img_path
            ...
            replaced_choices = {str(k): replace_ph(v) for k, v in choices_pre.items()}  # 키는 원형숫자 유지
            ...
            # ✅ 정답을 선택지 스타일의 원형숫자로 치환해 문자열로 변환
            ...
            ai = {
                ...
                **({"문제보기": replaced_view} if replaced_view else {}),
                "선택지": choices_for_addinfo,  # 원형숫자 키 유지
                "정답": answer_text,
